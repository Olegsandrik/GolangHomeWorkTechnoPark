...
func TestTableParserSuccess(t *testing.T) {
	for i := 0; i < len(tableTests); i++ {
		result, err := Uniq(tableTests[i].in.Strings, tableTests[i].in.Option)
		if err != nil {
			t.Fatalf("%s", err)
		}
		require.Equal(t, tableTests[i].out, result, "The two slices should be the same.")
	}
